...
Las cuatro características básicas de la programación orientada a objetos son encapsulación, abstracción, herencia y polimorfismo. Estas características definen la forma en que se estructuran los programas y cómo interactúan sus componentes, normalmente usados en C.

Encapsulación: consiste en agrupar datos y operaciones relacionadas dentro de una misma unidad llamada clase, controlando además el acceso a esos datos.  
Abstracción: permite centrarse en qué hace un objeto y no en cómo lo hace internamente, mostrando solo la información necesaria, al resto del programa y ocultando los detalles de implementación.
Herencia: Permite crear nuevas clases a partir de otras ya existentes, reutilizando código y estableciendo relaciones jerárquicas entre clases. 
Polimorfismo: Este posibilita que diferentes objetos respondan de manera distinta ante el mismo mensaje u operación, lo que aporta flexibilidad y facilita la extensión del software sin modificar código ya existente.

## 2. Cita cuatro lenguajes populares que permitan la programación orientada a objetos

Existen numerosos lenguajes de programación que permiten aplicar programación orientada a objetos, que son ampliamente utilizados en todos los ambitos. Estos lenguajes proporcionan mecanismos nativos para definir clases, crear objetos y aplicar conceptos como herencia o polimorfismo.
Java: Uno de los lenguajes orientados a objetos más conocidos y utilizados, especialmente en aplicaciones empresariales, educativas y de desarrollo multiplataforma. 
C++: Permite la programación orientada a objetos, aunque combina este paradigma con características de C, lo que da lugar a un estilo híbrido.
Python: Soporta orientación a objetos de forma flexible y sencilla. 
C#: muy utilizado en el entorno de desarrollo de aplicaciones de escritorio, web y videojuegos dentro del ecosistema de Microsoft. Todos ellos permiten estructurar programas basados en clases y objetos, aunque con diferencias en sintaxis y filosofía de diseño.
...
Estado: Representa la información que almacena el objeto en un momento dado. Este estado se define mediante variables internas, llamadas atributos, que contienen los datos propios del objeto y pueden variar a lo largo de su ciclo de vida.
Comportamiento: Describe las acciones que el objeto puede realizar o las operaciones que se pueden aplicar sobre él. Este comportamiento se implementa mediante métodos o funciones asociadas al objeto.
Identidad: Permite distinguir un objeto de otro incluso aunque tengan el mismo estado y comportamiento, ya que cada objeto es una instancia única dentro del sistema.

## 5. ¿Qué es una clase? ¿Es lo mismo que un objeto? ¿Qué es una instancia? ¿Todos los lenguajes orientados a objetos manejan el concepto de clase?

Una clase describe cómo serán los objetos de un determinado tipo. En ella se especifican los atributos que tendrán y los métodos que podrán ejecutar, pero la clase en sí no representa un elemento concreto del programa, sino un modelo a partir del cual se crean objetos.
...
No todos los lenguajes orientados a objetos manejan el concepto de clase de la misma forma. La mayoría, como Java o C++, se basan directamente en clases, pero existen lenguajes orientados a objetos que utilizan otros enfoques, como los basados en prototipos, donde los objetos se crean a partir de otros objetos sin necesidad de una clase formal.

## 6. ¿Dónde se almacenan en memoria los objetos? ¿Es igual en todos los lenguajes? ¿Qué es la **recolección de basura**? 

En programación orientada a objetos, los objetos se almacenan en memoria dinámica, también conocida como heap. A diferencia de las variables locales simples, los objetos suelen persistir más allá del bloque donde se crean, siempre que exista alguna referencia a ellos.
...
A continuación se muestra un ejemplo mínimo de clase en Java que cumple las condiciones indicadas. La clase se llama `Punto`, tiene dos atributos `x` e `y`  un método `calculaDistanciaAOrigen` que devuelve la distancia desde el punto hasta el origen `(0,0)` usando la fórmula matemática habitual.
class Punto {
    double x;
    double y;
...
En este ejemplo, la clase define únicamente la estructura y el comportamiento del punto, pero no crea ningún objeto por sí misma. Los atributos `x` e `y` representan el estado del objeto, mientras que el método define una operación que utiliza ese estado.

El siguiente fragmento muestra un ejemplo donde se crea una instancia de la clase `Punto`, se asignan valores a sus atributos y se invoca el método definido anteriormente:
...
La combinación de `static` y `final` se utiliza habitualmente para definir valores constantes asociados a una clase. `static` permite que el valor exista una sola vez y sea compartido, mientras que `final` impide que dicho valor sea modificado una vez inicializado. Esta combinación es común para definir constantes globales.
...
La referencia `this` es una variable  que existe dentro de los métodos de una clase y que apunta al objeto actual. Su uso permite acceder a los atributos y métodos del propio objeto, especialmente cuando existe ambigüedad entre variables locales o parámetros y los atributos de la clase.

Uno de los usos más habituales de `this` se da cuando los parámetros de un método o constructor tienen el mismo nombre que los atributos del objeto. En ese caso, `this` permite diferenciar  que se está accediendo al atributo del objeto y no a la variable local. También puede utilizarse para pasar el propio objeto como argumento a otro método o para encadenar constructores dentro de una misma clase.

La referencia no se llama igual en todos los lenguajes orientados a objetos. En Java, C++ y C# se utiliza el nombre `this`, mientras que en otros lenguajes puede recibir nombres distintos (por ejemplo, `self` en Python). El concepto, es el mismo: una referencia al objeto actual.
...
El método devuelve un valor numérico que representa dicha distancia, sin modificar el estado de ninguno de los objetos implicados.
...
La idea de disponer de un método que convierta un objeto en texto existe también en otros lenguajes orientados a objetos, aunque con nombres distintos. Por ejemplo, en C++ se puede lograr un comportamiento similar mediante sobrecarga de operadores, y en Python existe el método `__str__`.
...
Lo que le falta al `struct` para ser como una clase es, en primer lugar, la integración directa del comportamiento con los datos. En C, las funciones que operan sobre un `struct` están separadas de la definición de los datos, mientras que en una clase los métodos forman parte de la propia estructura. Esto permite que los datos y las operaciones estén cohesionados y se acceda a ellos de forma controlada.
